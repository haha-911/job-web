<template>
  ...
  <el-table :data="tableData" style="width: 100%" border @selection-change="handleSelectionChange">
    ...
    <el-table-column label="职位发布时间" prop="applyTime">
    </el-table-column>
  ...
  <el-dialog title="请填写信息" v-model="editVisible" width="30%">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="面试时间">
        <el-date-picker v-model="form.time" type="date" placeholder="开始日期" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
          class="handle-input mr10">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="面试地点" prop="title">
...
const getApplyList = () => {
  ...
  api.getApply(requestData).then((result) => {
...
const handleSave = () => {
  ...
  api.passApply(form.value).then((result) => {
...
const handleRefuse = (val) => {
  ...
  api.refuseApply(ids).then((result) => {
    ElMessage.success(result.msg)
    getApplyList()
  }).catch((err) => {
